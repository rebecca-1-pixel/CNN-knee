...
# Funzione per l'aumento dei dati
...
# Definizione della funzione per l'aumento 3D'
def augment3D(coil_img, degree=([0, 0.01, 0.01]),
              translation=([0.05, 0, 0])):  # through-plane rotation in degree, translation in mm
...
def main(args):
    ...
    # Caricamento del modello della checkpoint
    ...
    cfg = net.cfg
    net.GT = args.GT
    # Creazione dei dataset per i volumi
        ...
        with torch.no_grad():
            if (args.aux_aug > 0) & (len(volume) > 1):
                ...
                volume_ref_rigid, volume_ref_rss_rigid = augment3D(volume_ref,
                                                                   degree=([0, args.aux_aug * args.rotate,
                                                                            args.aux_aug * args.rotate]),
                                                                   translation=([args.aux_aug * args.translate, 0,
            ...
            net.test(*batch)
            # Raccolta delle metriche e stampa
            ...
            for name, val in vis['images'].items():
                torchvision.utils.save_image(val,
                                             ...
                                             pad_value=0.5)  # range=(0, 1) removed
...
# Parsing degli argomenti da riga di comando
    ...
    parser.add_argument('--basepath', default='D:/tesi rebecca/jCAN-main',
