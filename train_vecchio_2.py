...
# Esegui nvidia-smi e stampa l'output
...
class Prefetch(torch.utils.data.Dataset):
    def __init__(self, dataset, device):  #accetta un dataset
        super().__init__()
        self.dataset = [i.to(device) for i in tqdm.tqdm(dataset, leave=False)]  #il costruttore crea una lista che contiene tutti
...
def main(main_args):
    ...
    cfg.use_amp = False  # Abilita la precisione mista
    ...
    for index_epoch in tqdm.trange(epoch_start, main_args.epoch, desc='epoch', leave=True):
        ...
        for batch in tqdm_iter:
            ...
            if iter_cnt % 100 == 0:  # Ogni 100 iterazioni, ad esempio, controlliamo la memoria
                print("\nMemory summary after iteration:", iter_cnt)
