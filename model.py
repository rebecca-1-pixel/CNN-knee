...
#MISURA DELLA PARDITA BASATA SUI GRADIENTI SPAZIALI
def gradient_loss(s):
    assert s.shape[-1] == 2, 'not 2D grid?'  #verifica che il tensore abbia due dimensioni
    dx = torch.abs(s[:, :, 1:, :] - s[:, :, :-1, :])  #calcolo del gradiente orizzontale
    dy = torch.abs(s[:, 1:, :, :] - s[:, :-1, :, :])  #calcolo del gradiente verticale
    #quadratura:
    dy = dy * dy
    dx = dx * dx
    d = torch.mean(dx) + torch.mean(dy)  #misura complessiva della variazione dei gradienti
    return d / 2.0  #normalizza


#LISTA DI VALORI BASATI SU PROGRESSIONE DECRESCENTE
def generate_rhos(num_recurrent):  #num_recurrent rappresenta il n° di passi o iterazioni per i quali si desidera
                                   #generare i valori
...
def tv_loss(img, weight):  #weight modula l'importanza della perdita di variazione totale
    bs_img, c_img, h_img, w_img = img.size()  #estrae le dimensioni del tensore
    tv_h = torch.pow(img[:, :, 1:, :] - img[:, :, :-1, :], 2).sum()  #perdita di variazione totale orizzontale
    tv_w = torch.pow(img[:, :, :, 1:] - img[:, :, :, :-1], 2).sum()  #perdita di variazione totale verticale
    return weight * (tv_h + tv_w) / (bs_img * c_img * h_img * w_img)  #perdita totale

#DIZIONARIO MASKS: mappa i nomi delle maschere a classi specifiche di maschere. è usato per scegliere la mask appropriata
masks = {"mask": Mask,
         "taylor": TaylorMask,
         "random": RandomMask,
         "lowpass": LowpassMask,
