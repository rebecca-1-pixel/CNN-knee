...
#PARTE MOFIFICATA!!!!!!!!!!!!!
  ...
  #FINE PARTE MODIFICATA!
    ...
    def pad_tensor(tensor, target_depth):
        d, h, w = tensor.shape
        pad = (0, 0, 0, 0, 0, target_depth - d)  # Padding sulla prima dimensione
        return F.pad(tensor, pad, mode='constant', value=0)

    def collate_fn(batch):
        max_depth = max(x.shape[0] for x in batch)  # Trova la profondità massima nel batch
        batch = [pad_tensor(x, max_depth) for x in batch]  # Applica il padding a tutti i tensori
    ...
    for i, sample in enumerate(slices_val):
        print(f"Validation Sample {i} shape: {sample.shape}")

    slices_train = torch.utils.data.ConcatDataset(volumes_train)

    for i, sample in enumerate(slices_train):
        print(f"Training Sample {i} shape: {sample.shape}")
    ...
    for index_epoch in tqdm.trange(epoch_start, main_args.epoch, desc='epoch', leave=True):
        ...
        loader_val = torch.utils.data.DataLoader(
            ...
            num_workers=main_args.num_workers, pin_memory=True, drop_last=True,collate_fn=collate_fn
        )
