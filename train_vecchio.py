...
class Prefetch(torch.utils.data.Dataset):
    def __init__(self, dataset):  #accetta un dataset
        super().__init__()
...
def main(main_args):
    ...
    # Imposta l'attributo 'device' in cfg
    cfg.device = device.type  # Salva 'cuda:0' o 'cpu' in cfg
    print(cfg.device)

    # Se ci sono almeno 2 GPU, configura DataParallel
    if torch.cuda.device_count() >= 2:
        print("Using", torch.cuda.device_count(), "GPUs!")
        cfg.GPUs = 1
        device_ids = [0, 1]  # Specifica gli ID delle GPU da utilizzare
        # Inizializza il modello (creato con la configurazione cfg)

    else:
        print("Two GPUs are required but not available. Using CPU!")
        cfg.GPUs = 1
        net = ReconModel(cfg=cfg)
        net = net.to(device)  # Se non ci sono abbastanza GPU, usa la CPU


    net = ReconModel(cfg=cfg)
    net = net.to(device)  # Assicurati che il modello venga spostato sulla GPU scelta
    net = torch.nn.DataParallel(net, device_ids=device_ids)  # Imposta il modello su entrambe le GPU


    batchsize_train = main_args.batch_size
    iter_cnt = 0
    ...
    batch_vis = [x.cuda().to(device, non_blocking=True) for x in batch_vis]
    ...
    #use_amp = True  # Abilita la precisione mista
    ...
    #CICLO DI TRAINING
    #si inizia un ciclio che scorre su tutte le epoche di training
    for index_epoch in tqdm.trange(epoch_start, main_args.epoch, desc='epoch', leave=True):
        ...
        loader_train = torch.utils.data.DataLoader(slices_train, batch_size=batchsize_train, shuffle=True,
                                                   num_workers=main_args.num_workers, pin_memory=False, drop_last=True,
                                                   )
        ...
        for batch in tqdm_iter:
            ...
            for param in net.parameters():
                print(f"Parameter: {param.shape}, requires_grad: {param.requires_grad}, grad: {param.grad}")

            loss_all.mean().backward()  #backpropagation della loss scalata utilizzando la precisione mista

            optim_r.step()  #Aggiorna i pesi del modello usando l'ottimizzatore
            scalar.update()  #Aggiorna lo scaler utilizzato per la precisione mista

            del batch

            time_start = time.time()
            #SALVATAGGIO IMMAGINI DI RICOSTRUZIONE
            if iter_cnt % 5000 == 0:  #ogni 500 iterazioni...
                last_disp = iter_cnt
...
def parse_arguments():
    ...
    parser.add_argument('--basepath',
                        default='C:/Users/rebic/PycharmProjects/test_torch/jCAN-main', type=str,
                        help='path to basepath of data')
    parser.add_argument('--coils', type=int, default=24, help='number of coils')
...
print(torch.cuda.is_available()) #verifico innanzitutto che la GPU sia disponibile
    # SELEZIONE DEL DISPOSITIVO (GPU/CPU)
    device = torch.device('cuda:0')
    torch.cuda.set_device(0)
...
class Prefetch(torch.utils.data.Dataset):
    def __init__(self, dataset, device):  #accetta un dataset
        super().__init__()
        self.dataset = [i.to(device).half() for i in tqdm.tqdm(dataset, leave=False)]  #il costruttore crea una lista che contiene tutti
                                                                     #gli elementi del dataset precaricati in memoria
        self.device = device
...
def main(main_args):
    ...
    print(torch.cuda.is_available())  #verifico che la GPU sia disponibile
    ...
    if torch.cuda.is_available():
        if torch.cuda.device_count() >= 2:
            ...
            device_ids = [0, 1]  # Specifica gli ID delle GPU da utilizzare
            device = torch.device('cuda:0')
            torch.cuda.set_device(0)  # Imposta la prima GPU come predefinita
        else:
            print("Only one GPU available. Using it!")
            cfg.GPUs = 1
            device_ids = [0]  # Usa solo la GPU disponibile
            device = torch.device('cuda:0')
            torch.cuda.set_device(0)  # Imposta la GPU disponibile
    else:
    ...
    net = ReconModel(cfg=cfg)
    net = net.to(device)  # Assicurati che il modello venga spostato sul dispositivo configurato

    # Configura il modello per multi-GPU se ci sono almeno due GPU
    if cfg.GPUs >= 2:
        print("Usando DataParallel con 2 GPU!")
        net = torch.nn.DataParallel(net, device_ids=device_ids)  # Usa entrambe le GPU
    else:
    ...
    #CONFIGURA OTTIMIZZATORE E SCALER PER AMP
    optim_r = torch.optim.AdamW(net.parameters(), lr=cfg.lr, weight_decay=0)
    cfg.use_amp = True  # Abilita la precisione mista
    scalar = torch.amp.GradScaler('cuda',enabled=cfg.use_amp)
    ...
    for index_epoch in tqdm.trange(epoch_start, main_args.epoch, desc='epoch', leave=True):
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=cfg.use_amp):
                for batch in tqdm_iter:
                    time_data = time.time() - time_start
                    batch = [x.to(device, non_blocking=True).half() for x in batch]
                    batch = [utils.complex_to_chan_dim(x) for x in batch]
